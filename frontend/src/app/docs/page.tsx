...
export default function DocsPage() {
  return (<main className="min-h-screen max-w-3xl mx-auto px-4 py-10 space-y-8">
    ...
    <div className="space-y-6 text-sm text-zinc-400 leading-relaxed">
      ...
      <section className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 space-y-2"><h2 className="text-lg font-semibold text-zinc-200">3. Install the fix</h2><div className="grid grid-cols-1 md:grid-cols-2 gap-3"><div className="bg-zinc-800/50 rounded-lg p-3"><div className="font-medium text-zinc-200">Shopify</div><div className="text-zinc-500 text-xs mt-1">Install our App from Shopify App Store. One click, all products get Schema.</div><Link href="/api/shopify/install?shop=yourstore.myshopify.com" className="text-xs text-emerald-400 mt-2 inline-block">Connect →</Link></div><div className="bg-zinc-800/50 rounded-lg p-3"><div className="font-medium text-zinc-200">WordPress</div><div className="text-zinc-500 text-xs mt-1">Upload the plugin. Activates Schema automatically.</div><Link href="/wordpress" className="text-xs text-emerald-400 mt-2 inline-block">Get Plugin →</Link></div></div></section>
