...
function OptimizeContent() {
  ...
  return (<main className="min-h-screen max-w-4xl mx-auto px-4 py-10 space-y-6">
    ...
    <div className="flex gap-3"><input value={url} onChange={e => setUrl(e.target.value)} placeholder="Product URL..." className="flex-1 px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" /><button onClick={run} disabled={loading} className="px-6 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-medium rounded-lg transition">{loading ? "..." : "Generate Fixes"}</button></div>
