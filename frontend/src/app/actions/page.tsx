...
function ActionCenter() {
  ...
  return (<main className="min-h-screen max-w-5xl mx-auto px-4 py-10 space-y-8">
    ...
    {actions.length>0&&<div className="space-y-8">{tiers.map(t=>{const items=grouped[t.key];if(!items.length)return null;return(<section key={t.key} className={`${t.bg} border ${t.border} rounded-2xl p-6`}><div className="flex items-center gap-2 mb-4"><span className="text-xl">{t.icon}</span><h2 className={`text-lg font-semibold ${t.color}`}>{t.label}</h2><span className="text-xs text-zinc-500 ml-2">({items.length} issues)</span></div><div className="space-y-3">{items.map((a:any,i:number)=>(<div key={i} className={`bg-zinc-900/60 rounded-lg p-4 flex items-start justify-between ${i>=3?'opacity-30 blur-[2px] pointer-events-none select-none':''}`}><div className="flex-1"><div className="text-sm font-medium text-zinc-200">{a.what_to_do}</div><div className="text-xs text-zinc-500 mt-1">{a.how_to_do_it?.slice(0,150)}</div></div><div className="shrink-0">{t.key==="auto"?<Link href={`/optimize?domain=${encodeURIComponent(domain)}`} className="text-xs px-3 py-1 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg transition">Fix Now</Link>:t.key==="guided"?<span className="text-xs text-amber-400">💡 View Guide</span>:<Link href="/monitoring" className="text-xs px-3 py-1 bg-zinc-700 hover:bg-zinc-600 text-zinc-300 rounded-lg transition">Monitor</Link>}</div></div>))}{items.length>3&&<div className="text-center py-3 bg-zinc-900/80 rounded-lg border border-emerald-800"><p className="text-sm text-emerald-400">🔒 {items.length-3} more issues hidden. <Link href="/signup" className="underline">Sign up free</Link> to unlock all.</p></div>}</div></section>)})}</div>}
