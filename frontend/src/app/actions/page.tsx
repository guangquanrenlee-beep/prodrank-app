...
export default function ActionCenterPage() {
  ...
  const scanDomain = async (d: string) => { if (!d.trim()) return; setLoading(true); try { const r = await fetch("/api/next-steps", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ url: `https://${d.trim()}`, product_name: d.trim() }) }); if (r.ok) { const data = await r.json(); setActions(data.action_plan || []); } } catch {} setLoading(false); };
  ...
  actions.forEach(a => { grouped[getTier(a)].push(a); });
  ...
  return (<main className="min-h-screen max-w-5xl mx-auto px-4 py-10 space-y-8">
    ...
    {actions.length > 0 && (<div className="space-y-8">{tiers.map(tier => { const items = grouped[tier.key]; if (!items.length) return null;
        return (<section key={tier.key} className={`${tier.bg} border ${tier.border} rounded-2xl p-6`}>
          ...
          <div className="space-y-3">{items.map((a, i) => (<div key={i} className="bg-zinc-900/60 rounded-lg p-4 flex items-start justify-between gap-4">
    ...
    {!loading && actions.length === 0 && <div className="text-center py-16 text-zinc-500">Enter your domain to see your action plan — or <Link href="/dashboard" className="text-emerald-400 underline">go back to Dashboard</Link> to re-scan.</div>}
  </main>);
}
