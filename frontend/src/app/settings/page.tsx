...
export default function SettingsPage() {
  ...
  useEffect(() => { supabase.auth.getSession().then(({ data: { session } }) => { setUser(session?.user ?? null); setEmail(session?.user?.email ?? ""); }); }, []);
  ...
  return (
    <div className="min-h-screen bg-zinc-950 flex">
      <aside className="w-56 bg-zinc-900 border-r border-zinc-800 shrink-0 flex flex-col p-4">
        <Link href="/" className="font-bold text-emerald-400 text-lg mb-6">ProdRank</Link>
